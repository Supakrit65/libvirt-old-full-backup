...
#
# Usage: vm_backup.sh --backup-base-dir DIR --vm-domain NAME [--max-backups N] [--compress] [--new-vm-domain-name NAME]
...
usage() {
    cat <<EOF
Usage: $(basename "$0") --backup-base-dir DIR --vm-domain NAME [--max-backups N] [--compress] [--new-vm-domain-name NAME]
Options:
  ...
  --new-vm-domain-name NAME   Custom name for restored VM domain (default: <vm>-restore-<timestamp>)
