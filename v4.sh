...
if [[ "$STATE" == "running" ]]; then
  ...
  # omit --no-metadata so that snapshot metadata is preserved
...
# 3) Merge overlays back into base and remove overlays + metadata (if running)
if [[ "$STATE" == "running" ]]; then
...
# 4) Copy each merged base disk to backup folder (optionally compress)
echo "  Copying disks:"
