...
# 9) Define new backup VM and inject new UUID into snapshot XMLs
echo "[$(date '+%F %T')] Defining new backup VM from $XML_FILE…"
if virsh define "$XML_FILE"; then
  echo "   ✔ Domain defined successfully."
else
  echo "   ✘ Failed to define domain from XML!" >&2
  exit 1
fi

# Get the UUID of the newly defined VM
NEW_UUID=$(virsh domuuid "$RESTORED_NAME")
...
if [[ -n "$INTERNAL_SNAPSHOTS" ]]; then
  ...
  for SNAPSHOT_XML in "$DEST/snapshots/"*.xml; do
    ...
    # Redefine the snapshot in the new VM
    if virsh snapshot-create "$RESTORED_NAME" --xmlfile "$SNAPSHOT_XML" --redefine; then
      echo "     ✔ Snapshot '$SNAPSHOT_NAME' redefined for $RESTORED_NAME"
    else
      echo "     ✘ Failed to redefine snapshot '$SNAPSHOT_NAME'" >&2
    fi
  done
