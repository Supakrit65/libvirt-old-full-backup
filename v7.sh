...
if [[ -n "$INTERNAL_SNAPSHOTS" ]]; then
  ...
  # Iterate through each snapshot name
  while IFS= read -r SNAPSHOT; do
    echo "    → Migrating snapshot: $SNAPSHOT"
    SNAPSHOT_XML="$DEST/snapshots/${SNAPSHOT}.xml"

    # Dump snapshot XML to file
    virsh snapshot-dumpxml "$VM" "$SNAPSHOT" > "$SNAPSHOT_XML"

    # Patch domain and inactiveDomain name/uuid with restore suffix
    xmlstarlet ed -L \
      -u "/domainsnapshot/domain/name" -v "$RESTORED_NAME" \
      -u "/domainsnapshot/domain/uuid" -v "" \
      -u "/domainsnapshot/inactiveDomain/name" -v "$RESTORED_NAME" \
      -u "/domainsnapshot/inactiveDomain/uuid" -v "" \
      "$SNAPSHOT_XML"

    # Update disk paths inside snapshot XML
    for i in "${!DISK_TARGETS[@]}"; do
      dev=${DISK_TARGETS[i]}
    ...
    echo "    ✔ Snapshot '$SNAPSHOT' migrated to $SNAPSHOT_XML"
...
# 7) Patch the domain XML metadata
#    a) Replace or insert <uuid> with a new one
#    b) Replace <name> with RESTORED_NAME
#    c) Update each disk <source file='...'> path to point to the new backup location

NEW_UUID=$(uuidgen)
echo "[$(date '+%F %T')] Patching VM domain XML with name: $RESTORED_NAME and uuid: $NEW_UUID"

# Step 7a: Replace <name> and <uuid> (or insert if missing)
if xmlstarlet sel -t -v "/domain/uuid" "$XML_FILE" &>/dev/null; then
  # UUID exists → update it
  xmlstarlet ed -L \
    -u "/domain/name" -v "$RESTORED_NAME" \
    -u "/domain/uuid" -v "$NEW_UUID" \
    "$XML_FILE"
else
  # UUID missing → insert it after <name>
  xmlstarlet ed -L \
    -u "/domain/name" -v "$RESTORED_NAME" \
    -s "/domain/name" -t elem -n uuid -v "$NEW_UUID" \
    "$XML_FILE"
fi

# Step 7c: Update disk <source file='...'> paths
for i in "${!DISK_TARGETS[@]}"; do
  dev=${DISK_TARGETS[i]}
  old_path=${DISK_SRCS[i]}
  new_path="$DEST/${dev}.qcow2"

  echo "    ↳ Updating disk '$dev' path: $old_path → $new_path"
...
# Get the UUID of the newly defined VM
echo "   → New UUID: $NEW_UUID"

# Inject UUID into all migrated internal snapshot XMLs and redefine snapshots
if [[ -n "$INTERNAL_SNAPSHOTS" ]]; then
  echo "   ↻ Updating snapshot XMLs with new UUID and redefining snapshots…"

  for SNAPSHOT_XML in "$DEST/snapshots/"*.xml; do
    SNAPSHOT_NAME=$(basename "$SNAPSHOT_XML" .xml)
    ...
    xmlstarlet ed -L \
      ...
      "$SNAPSHOT_XML"
    echo "     • Patched UUID in $SNAPSHOT_NAME.xml"
  done

  echo "   ✔ All snapshots updated and redefined."
fi